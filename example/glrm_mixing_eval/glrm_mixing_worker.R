...
glrm_worker <- 
  function(
    ...
    PRIOR = c("gaussian", "sparse", "sparse_plus"),
          ...
          for (k_true in K_true){
            for (k_model in K_model){
              for (lambda in LAMBDA){
                ...
                rec <- NULL
                init_path <- "./result/init/"
                init_name <- 
                  paste0(init_path, family_name, "_", prior_name,
                         "_ktr", k_true, "_kmd", k_model,
                         "_snr", snr, "_", data_seed, "_", 
                         trial_seed, ".RData")
                
                for (samplr_name in SAMPLR){
                  
                  if (samplr_name == "vi_stan"){
                    # if (is.null(rec$init)){
                    init_MAP <- FALSE
                    init <- NULL
                    # init$V <- t(data.sim$V)
                    ...
                    # if hmc_stan, use init generated by vi_stan
                    init_MAP <- FALSE
                    load(init_name)
                  }
                  ...
                  if (save_error) {
                    ...
                    # add to existing file
                    rec_error <- 
                      matrix(c(0, rec$pred_error), nrow = 1)
                  ...
                  if (samplr_name == "vi_stan") {
                    if (!dir.exists(init_path)){
                      dir.create(init_path)
                    }
                    
                    # modify init and save for hmc to use
                    init$U <- rec$U[2, , ]
                    init$V <- rec$V[2, , ]
                    
                    save(init, file = init_name)
                  }
                }
              }
            }
...
# for (i in 1:k_model){
#   #lines(apply(rec$V[, , i], 1, sd), col = i)
...
# sapply(1:iter_max[2],
#        function(i)
#          sum(rec$gamma_rank_cumprod[i, , ] > 5e-2)
